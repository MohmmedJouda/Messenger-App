...
const conversation = function (chat) {
    $("#chat-list").append(`<a href="#${chat.id}" data-messages="${
        chat.id
...
$("#chat-list").on("click", "[data-messages]", function (e) {
    ...
    $.get(`/api/conversations/${id}/messages`, function (response) {
        ...
        $("#chat-avatar").attr(
            "src",
            response.conversation.participants[0].avatar_url
        );
        for (i in response.messages.data.reverse()) {
            let msg = response.messages.data[i];
            let c = msg.user_id == userId ? "message-out" : "";

            addMessage(msg, c);
            // console.log(response.messages.data[i]);
        }
    });
});
